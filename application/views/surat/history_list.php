<div class="box">
		...
		<th>Action</th>
            ...
            <td><?php echo $surat->nama_depan.' '.$surat->nama_belakang ?></td>
        ...
        if($this->session->userdata('jabatan') == 'Kepala Desa') {
          if($surat->status_persetujuan == 'Belum disetujui'){
        echo anchor(site_url($setujui.$surat->kd_surat),'Setujui', 'onclick="javasciprt: return confirm(\'Apakah Anda Ingin Menyetujui Surat Ini?\')", class="btn btn-xs btn-danger"');
          }else{
          echo anchor(site_url($batal.$surat->kd_surat),'Batalkan Persetujuan', 'onclick="javasciprt: return confirm(\'Apakah Anda Ingin Membatalkan Menyetujui Surat Ini?\')", class="btn btn-xs btn-warning"');
          echo "<br>";
          echo anchor(site_url($cetak.$surat->kd_surat),'CETAK', 'class="btn btn-xs btn-success"');
        }
        ...
        if($surat->status_persetujuan == 'Belum disetujui'){
          echo "<p style='color:red'>Belum di setujui<br>tidak dapat cetak</p>";
        }else{
				echo anchor(site_url($cetak.$surat->kd_surat),'CETAK', 'class="btn btn-xs btn-success"');
      }
    }
